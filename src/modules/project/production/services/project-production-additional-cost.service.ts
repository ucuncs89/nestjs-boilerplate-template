...
@Injectable()
export class ProjectProductionAdditionalCostService {
  constructor(
  ...
  async create(
    project_id: number,
    projectAdditionalCostDto: ProjectProductionAdditionalCostDto,
    user_id: number,
  ) {
    try {
  ...
  async findAdditionalProduction(project_id: number) {
    const data = await this.projectAdditionalCostRepository.find({
      select: {
        id: true,
        additional_name: true,
