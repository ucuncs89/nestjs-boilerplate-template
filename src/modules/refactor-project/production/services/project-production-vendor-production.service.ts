...
import {
  ...
  ProjectProductionVendorProductionLossPercentageDto,
} from '../dto/project-production-vendor-production.dto';

@Injectable()
export class ProjectProductionVendorProductionService {
  constructor(
  ...
  async updateLossPercentage(
    project_detail_id,
    projectProductionVendorProductionLossPercentageDto: ProjectProductionVendorProductionLossPercentageDto[],
    user_id,
    i18n,
  ) {
    ...
    try {
      if (
        Array.isArray(projectProductionVendorProductionLossPercentageDto) &&
        projectProductionVendorProductionLossPercentageDto.length > 0
      ) {
        for (const data of projectProductionVendorProductionLossPercentageDto) {
      ...
      return projectProductionVendorProductionLossPercentageDto;
    } catch (error) {
  ...
  async updateVendorProductionDueDate(
    ...
    try {
      const data = await this.projectVendorProductionDetailRepository.update(
        ...
        {
          production_due_date:
            projectProductionVendorProductionDetailDueDateDto.production_due_date,
          updated_at: new Date().toISOString(),
          updated_by: user_id,
        },
      );
