...
@Injectable()
export class ProjectSamplingPurchaseOrderService {
  constructor(
  ...
  async createPurchaseOrder(
    project_detail_id,
    projectSamplingPurchaseOrderDto: ProjectSamplingPurchaseOrderDto,
    user_id,
  ) {
    ...
    try {
      ...
      await queryRunner.manager.insert(ProjectPurchaseOrderEntity, {
        ...
        created_by: user_id,
        vendor_type: projectSamplingPurchaseOrderDto.vendor_type,
        relation_id: projectSamplingPurchaseOrderDto.relation_id,
        material_type: projectSamplingPurchaseOrderDto.material_type,
        vendor_id: projectSamplingPurchaseOrderDto.vendor_id,
      });
  ...
  async updatePurchaseOrder(
    ...
    try {
      await this.purchaseOrderRepository.update(
        ...
        {
          vendor_id: projectSamplingPurchaseOrderDto.vendor_id,
          company_name: projectSamplingPurchaseOrderDto.company_name,
          company_address: projectSamplingPurchaseOrderDto.company_address,
          company_phone_number:
            projectSamplingPurchaseOrderDto.company_phone_number,
          ppn: projectSamplingPurchaseOrderDto.ppn,
          pph: projectSamplingPurchaseOrderDto.pph,
          discount: projectSamplingPurchaseOrderDto.discount,
          bank_account_number:
            projectSamplingPurchaseOrderDto.bank_account_number,
          bank_account_houlders_name:
            projectSamplingPurchaseOrderDto.bank_account_houlders_name,
          notes: projectSamplingPurchaseOrderDto.notes,
          updated_at: new Date().toISOString(),
          updated_by: user_id,
          payment_term: projectSamplingPurchaseOrderDto.payment_term,
        },
      );
      return { id: purchase_order_id, ...projectSamplingPurchaseOrderDto };
    } catch (error) {
      throw new AppErrorException(error);
    }
  }
